/** @scratch /panels/5
 *
 * include::panels/statsmap.asciidoc[]
 */

/** @scratch /panels/statsmap/0
 *
 * == Map
 * Status: *Stable*
 *
...
function (angular, app, _, $) {
  ...
  var module = angular.module('kibana.panels.statsmap', []);
  app.useModule(module);

  module.controller('statsmap', function($scope, $rootScope, querySrv, dashboard, filterSrv) {
    $scope.panelMeta = {
      editorTabs : [
    ...
    var _d = {
      /** @scratch /panels/statsmap/3
       *
       * === Parameters
       *
       * statsmap:: Map to display. world, usa, europe
       */
      statsmap     : "world",
      /** @scratch /panels/statsmap/3
    ...
    $scope.get_data = function() {
      ...
      request = request
        .facet($scope.ejs.TermsFacet('statsmap')
      ...
      results.then(function(results) {
        ...
        $scope.data = {};
        _.each(results.facets.statsmap.terms, function(v) {
    ...
    return {
      ...
      link: function(scope, elem) {
        ...
        function render_panel() {
          ...
          $('.jvectorstatsmap-zoomin,.jvectorstatsmap-zoomout,.jvectorstatsmap-label').remove();
          require(['./panels/statsmap/lib/map.'+scope.panel.map], function () {
            elem.vectorMap({
              map: scope.panel.map,
              ...
              onRegionLabelShow: function(event, label, code){
                elem.children('.statsmap-legend').show();
                var count = _.isUndefined(scope.data[code]) ? 0 : scope.data[code];
                elem.children('.statsmap-legend').text(label.text() + ": " + count);
              },
              onRegionOut: function() {
                elem.children('.statsmap-legend').hide();
              },
            ...
            elem.prepend('<span class="statsmap-legend"></span>');
